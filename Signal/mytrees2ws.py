...
def get_options():
    ...
    parser.add_option("--btag_config",type='string',dest="btag_config",default='/afs/cern.ch/user/f/fmonti/work/flashggFinalFit2/CMSSW_7_4_7/src/flashggFinalFit/MetaData_HHbbgg/btagSF_15_01_2019.json')
 ...
 for benchmark_num in whichNodes:
   ...
   for syst in systematics[0] : 
      for cat_num,cat in enumerate(cats) : 
         print 'doing cat ',cat
         refcat_for_btagrew = input_files[num]
         if("thq" in refcat_for_btagrew):
             refcat_for_btagrew = refcat_for_btagrew.replace("thq","tth")
         btag_renorm  = btag_normalization_dict["%s%s"%(refcat_for_btagrew,year)]
         ...
         if (tfile.Get("tagsDumper/trees/%s"%name).GetEntries())!=0 :
            ...
            if ('DoubleHTag' in cat ) : data = data.query("(leadingJet_pt_Mjj>0.55)")  #1/2.5 for all categories
