...
void OptionParser(int argc, char *argv[]){
  ...
  if (vm.count("forceFracUnity")) forceFracUnity_=true;
	...
	split(filenameVector_,filename_,boost::is_any_of(","));
...
int main(int argc, char *argv[]){
  ...
  float itsOK       = 0.1; // if  myDistance < itsOK suggest the #gauss that fulfill this condition
   ...
   vector<int> massList_;  // to be able to run with the new definition of the InitialFit
  ...
  for (int cat=0; cat<ncats_; cat++){
	...
	//	data   = reduceDataset((RooDataSet*)inWS->data(Form("%s_%d_13TeV_%s",proc.c_str(),mass_,flashggCats_[cat].c_str())));
		data   = reduceDataset((RooDataSet*)inWS->data(
          Form("%s_13TeV_%d_%s",proc.c_str(),mass_,flashggCats_[cat].c_str())));
//	data   = reduceDataset((RooDataSet*)inWS->data(Form("%s_%s",procWmassAndE.c_str(),flashggCats_[cat].c_str()) ) );
	...
	InitialFit initFitRV(mass,MH,mhLow,mhHigh,skipMasses,binnedFit,nBinsFit,massList_);
	...
	order++;
	  ...
	  if (rv_results[i].second < itsOK){
	    rvChoice = rv_results[i].first;	
		cout <<"inside break, distance is less than min distance "<<endl;  
	    break;
	  }
	  // look for the "significantly" smaller distance, where significantly is set by myThresholdDist 
      	  if ((rv_results[i].second < minDist*myThresholdDist) && (rv_chi2[i].second < minchi2*myThresholdChi2)){
				cout <<"inside first if "<<rv_results[i].first<<"  "<<rv_results[i].second<<endl;  
      	    minDist  = rv_results[i].second;
	    minchi2  = rv_chi2[i].second;
      	    rvChoice = rv_results[i].first;	  
				cout <<"inside first if rv Choice"<<rvChoice<<endl;  
      	  } 
	  else // look for the "significantly" smaller chi2, where significantly is set by myThresholdChi2
	    if ((rv_chi2[i].second < minchi2*myThresholdChi2) && (rv_results[i].second < minDist*myThresholdDist)){
	...
	InitialFit initFitWV(mass,MH,mhLow,mhHigh,skipMasses,binnedFit,nBinsFit,massList_);
